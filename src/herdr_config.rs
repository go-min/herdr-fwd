...
pub struct DashboardSetupStatus {
    pub sidebar_ports_enabled: bool,
    pub toast_delivery: Option<String>,
    pub popup_shortcut_enabled: bool,
    pub theme_name: Option<String>,
}

pub fn enable_ports_row(
    default_config: impl FnOnce() -> Result<String, String>,
) -> Result<PathBuf, String> {
    update_config(|config| update_ports_row(config, true, default_config))
}

pub fn set_ports_row(
    enabled: bool,
    default_config: impl FnOnce() -> Result<String, String>,
) -> Result<DashboardSetupStatus, String> {
    update_config(|config| update_ports_row(config, enabled, default_config))?;
    dashboard_setup_status()
}

pub fn dashboard_setup_status() -> Result<DashboardSetupStatus, String> {
...
pub fn enable_herdr_notifications() -> Result<DashboardSetupStatus, String> {
    update_config(update_herdr_notifications)?;
    dashboard_setup_status()
}

pub fn enable_dashboard_popup_shortcut() -> Result<DashboardSetupStatus, String> {
...
fn update_config(
    ...
    let path = herdr_config_path()?;
    // A stable lock file serializes settings from simultaneous companions;
    // locking config.toml itself would not survive its atomic replacement.
    let parent = path
        .parent()
    ...
    let lock = fs::OpenOptions::new()
        ...
        .read(true)
        .write(true)
        .open(path.with_extension("toml.herdr-fwd.lock"))
    ...
    crate::atomic::write_file(&path, updated.as_bytes(), 0o600)?;
    Ok(path)
}
...
fn add_ports_row_to_config(config: &str) -> Result<String, String> {
    update_ports_row(config, true, || {
        Err("test requires explicit sidebar rows".into())
    })
}
...
fn update_ports_row(
    ...
    let default_rows = if enabled && sidebar_rows_or_error(&original)?.is_none() {
        Some(herdr_default_space_rows(&default_config()?)?)
    } else {
        None
    };
...
fn herdr_default_space_rows(default_config: &str) -> Result<Array, String> {
...
fn dashboard_setup_status_from_document(document: &DocumentMut) -> DashboardSetupStatus {
    ...
    DashboardSetupStatus {
        ...
        theme_name: theme_name_from_document(document),
